...
def classify_field(career_text: str) -> str:
    ...
    # medical student counts as MEDICINE
...
def gym_mentioned(full_text: str) -> Tuple[int, str]:
    if not full_text:
        return 0, ""
    m = RE_GYM.search(full_text)
    if not m:
        return 0, ""
    return 1, m.group(0)
...
def main() -> None:
    ...
    for p in profiles:
        career_blob = extract_career_text(p)
        field = classify_field(career_blob)

        full = get_fulltext(p)
        hit, _ = gym_mentioned(full)
    ...
    labels = [FIELD_LABEL[k] for k in FIELDS_ORDER]
    pct: List[float] = []
    ...
    for k in FIELDS_ORDER:
        t = totals[k]
        h = hits[k]
        p = (h / t * 100.0) if t else 0.0
        pct.append(p)
        counts_text.append(f"{p:.1f}% ({h}/{t})")
    ...
    ax.set_title("Gym/exercise mentions by career field", loc="left")
    ...
    for i, (v, txt) in enumerate(zip(pct, counts_text)):
        ax.text(v + 0.6, i, txt, va="center", fontsize=10)

    ax.set_xlim(0, max(5, max(pct) * 1.18 if pct else 5))
